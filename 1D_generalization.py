...
def main():
    ...
    plot_compare(u_operator[0], ux_test[0], save_folder, option='train_l_0.5')    # select the first sample to plot

    # =============================================================================
    # TESTING FOR NEW PARAMETERS
    # =============================================================================
    # Put new parameters for the testing, l, k, D
    l = 0.3
    D_const = 0.2
    k = 0
    space = GRF(X, length_scale=l, N=1000, interp="cubic")
    ux_train, ux_v_train, v_train, D_train, t_train, x_train = DR_System(N_v_x, N_x, k, D_const, T, N_t, N_v_num, space, v_type, D_type, device).solve()
    node_new = SA_NODE(device, data_dim, dim_v, dim_u, hidden_dim, hidden_layers, 'relu', T, time_steps,
                   D_input=False, v_input=True).to(device)
    optimizer_adam_new = torch.optim.Adam(node_new.parameters(), lr=lr_adam)    # Define a new optimizer only for the NODE model
    optimizer_lbfgs_new = torch.optim.LBFGS(node_new.parameters(),     # Combine parameters
                                        lr = lr_lbfgs,
                                        max_iter = 20,
                                        max_eval = None,
                                        tolerance_grad = 1e-07,
                                        ...
                                        history_size = 100,
                                        line_search_fn = 'strong_wolfe')
    trainer_new = Trainer(node_new, basis, device, reg=reg_type, logger=logger)
    # Train the model with the new parameters
    trainer_new.train(ux_train, ux_v_train, v_train, D_train, x_train, t_train, optimizer_adam_new, optimizer_type='adam', num_epochs=num_epochs_adam)
    # trainer_new.train(ux_train, ux_v_train, v_train, D_train, x_train, t_train, optimizer_lbfgs_new, optimizer_type='lbfgs', num_epochs=num_epochs_lbfgs)
    # test the new model
    print("Space Generalization Testing")
    N_x = 100    # N_x is the number of spatial points
    N_t = 100    # N_t is the number of time points
    
    ux_test, ux_v_test, v_test, D_test, t_test, x_test = DR_System(N_v_x, N_x, k, D_const, T, N_t, N_v_num, space, v_type, D_type, device).solve()

    u_operator = trainer_new.compute_u_operator(ux_v_test, v_test, D_test, t_test, x_test)   # Compute the operator and squeeze the last dimension
    error = trainer_new.error_compute(ux_test, u_operator)    # Compute and print the error
    plot_compare(u_operator[0], ux_test[0], save_folder, option='test_l_0.3')    # select the first sample to plot
