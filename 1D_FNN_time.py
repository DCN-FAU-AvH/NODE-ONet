...
from models.space_basis import FNN, FNN_Time
...
def main():
    ...
    plot_compare(u_operator[0], ux_test[0], save_folder, option='FNN')    # select the first sample to plot

    # =============================================================================
    # FNN_Time
    # =============================================================================
    node_time = SA_NODE(device, data_dim, dim_v, dim_u, hidden_dim, hidden_layers, 'relu', T, time_steps,
                   D_input=False, v_input=True).to(device)
    basis_time = FNN_Time(device, data_dim, dim_u, hidden_dim, hidden_layers, 'relu', time_steps).to(device)
    optimizer_adam_time = torch.optim.Adam(list(node_time.parameters()) + list(basis_time.parameters()),       # Combine parameters
                                      lr=lr_adam)
    optimizer_lbfgs_time = torch.optim.LBFGS(list(node_time.parameters()) + list(basis_time.parameters()),     # Combine parameters
                                        lr = lr_lbfgs,
                                        max_iter = 20,
                                        max_eval = None,
                                        tolerance_grad = 1e-07,
                                        ...
                                        history_size = 100,
                                        line_search_fn = 'strong_wolfe')
    trainer_time = Trainer(node_time, basis_time, device, reg=reg_type, logger=logger)
    # train the model with the new parameters
    trainer_time.train(ux_train, ux_v_train, v_train, D_train, x_train, t_train, optimizer_adam_time, optimizer_type='adam', num_epochs=num_epochs_adam)
    # trainer_time.train(ux_train, ux_v_train, v_train, D_train, x_train, t_train, optimizer_lbfgs_time, optimizer_type='lbfgs', num_epochs=num_epochs_lbfgs)

    print("Testing FNN_Time...")
    N_t = 10
    N_x = 100
    T=1
    N_v_num = 1000
    ...
    u_operator = trainer_time.compute_u_operator(ux_v_test, v_test, D_test, t_test, x_test)    # Compute the operator and squeeze the last dimension
    error = trainer_time.error_compute(ux_test, u_operator)    # Compute and print the error
    plot_compare(u_operator[0], ux_test[0], save_folder, option='FNN_Time')    # select the first sample to plot
